...
from utils.llc_datetime import LLCMonth, get_previous_day, get_today
...
class DatabaseAPILayer:
    ...
    def __init__(self, client):
        ...
        self.engine = create_engine(
            ...
            pool_recycle=480,
        )  # 8 minutes, the estimate
        self.client = client
        self.logger = Logger(client)

    # Generalize all session commits behavior
    async def __commit(
        self, session, context, json_str_desc: str = None, array_desc: List[str] = None
    ):
        result = None
        try:
            session.commit()
            ...
            if json_str_desc is not None:
                await self.logger.on_db_update(
                    False, context, json.dumps(obj, default=str)
                )
            else:
                await self.logger.on_db_update(
                    False, context, json.dumps(obj, default=str)
    ...
    def __calculate_daily_object_delta(
        self,
        session: Session,
        user: db.User,
        daily_object: db.DailyObject,
        user_daily_object: db.UserDailyObject,
        submissions_list: List[dict],
    ):
        result = {
            "scoreEarned": 0,
            "scorePractice": 0,
        ...
        recorded_practice = (
            (user_daily_object.solvedEasy or 0) * score_config["Easy"]
        ...
        for submission in submissions_list:
            problem = problem_controller.read_one(
                session, titleSlug=submission["titleSlug"]
            )
            problem_type = ""
        ...
        with Session(self.engine, autoflush=False) as session:
            ...
            for fdom_f, fdom_blob in submissions_blob.items():
                ...
                for daily_f, daily_blob in fdom_blob.items():
                    ...
                    daily_object = daily_object_controller.read_one(
                        session, date=daily_d
                    )
                    ...
                    for username, submissions in daily_blob.items():
                        user = user_controller.read_one(
                            session, leetcodeUsername=username
                        )

                        user_monthly_object = user_monthly_object_controller.read_one(
                            session, user.id, fdom_d
                        )
                        if user_monthly_object is None:
                        ...
                        for sub in submissions:
                            problem = problem_controller.read_one(
                                session, titleSlug=sub["titleSlug"]
                            )
                            if problem is None:
                                continue
                            user_solved_problem = (
                                user_solved_problem_controller.read_one(
                                    session, user.id, problem.id
                                )
                            )
                            if (
                                daily_object.problemId != problem.id
                                and user_solved_problem is not None
                            ):
                        ...
                        daily_delta = self.__calculate_daily_object_delta(
                            session,
                            user,
                            daily_object,
                            user_daily_object,
                        ...
                        for sub in filtered_submissions:
                            ...
                            if user_solved_problem is None:
                                user_solved_problem_controller.create_one(
                                    session, user.id, problem.id, int(sub["id"])
                                )
                            if (
                                problem.id != daily_object.problemId
                                or user_daily_object.solvedDaily == 0
                                or user_daily_object.solvedDaily is None
                            ):  # hasn't registered daily submission yet
                                obj = {
                                    "submission": sub,
                                    "user": user.as_dict(),
                                    ...
                                    "info": {
                                        ...
                                        "is_daily": problem.id
                                        == daily_object.problemId,
                                    },
                                }
                                obj["problem"]["topics"] = [
                                    topic.topicName for topic in problem.topics
                                ]
                                result.append({"ObjType": "Submission", "Obj": obj})
                        # update and append changes to daily objects
                        daily_if_update = (
                            daily_delta["scoreEarned"]
                            + daily_delta["solvedDaily"]
                            + daily_delta["solvedEasy"]
                            + daily_delta["solvedMedium"]
                            + daily_delta["solvedHard"]
                        )
                        if daily_if_update:
                            if user_daily_object.id is None:
                                user_daily_object_controller.create_one(
                                    session=session,
                                    userId=user.id,
                                    dailyObjectId=daily_object.id,
                                    scoreEarned=daily_delta["scoreEarned"],
                                    solvedDaily=daily_delta["solvedDaily"],
                                    solvedEasy=daily_delta["solvedEasy"],
                                    solvedMedium=daily_delta["solvedMedium"],
                                ...
                                user_daily_object_controller.update_one(
                                    ...
                                    userId=user.id,
                                    dailyObjectId=daily_object.id,
                                    scoreEarnedDelta=daily_delta["scoreEarned"],
                                    solvedDailyDelta=daily_delta["solvedDaily"],
                                    solvedEasyDelta=daily_delta["solvedEasy"],
                                    solvedMediumDelta=daily_delta["solvedMedium"],
                        ...
                        if str(user.id) in user_score_earned_delta:
                            user_score_earned_delta[str(user.id)] += daily_delta[
                                "scoreEarned"
                            ]
                        else:
                            user_score_earned_delta[str(user.id)] = daily_delta[
                ...
                for userIdStr, scoreEarnedDelta in user_score_earned_delta.items():
                    ...
                    monthly_obj = user_monthly_object_controller.update_one(
                        ...
                        fdom=fdom_d,
                        scoreEarnedDelta=scoreEarnedDelta,
                    )
                    result.append(
                        {"ObjType": "UserMonthlyObject", "Obj": monthly_obj.as_dict()}
                    )
            await self.__commit(
                session, "RegisterNewCrawl", array_desc=api_utils.crawling_jstrs(result)
            )
        return result
    ...
    def read_user_progress(self, memberDiscordId: str):
        with Session(self.engine) as session:
            ...
            monthly_obj = ctrlers.UserMonthlyObjectController().read_one(
                session, user.id
            )
            daily_obj = ctrlers.DailyObjectController().read_one_or_latest(
                session, date=get_today()
            )
            user_daily_obj = ctrlers.UserDailyObjectController().read_one(
    ...
    def read_latest_daily_object(self):
        with Session(self.engine) as session:
            ...
            result["problem"]["topics"] = list(
                map(lambda topic: topic.topicName, problem.topics)
            )
        return result
    ...
    async def create_or_keep_daily_object(self, problemId, date):
        with Session(self.engine, autoflush=False) as session:
            ...
            if current_daily.generatedDate != date:
                new_obj = daily_object_controller.create(
                    session=session, problemId=problemId, date=date
                )
                session.add(new_obj)
                result = new_obj.as_dict()
            await self.__commit(session, "DailyObject", json.dumps(result, default=str))
        return result
    ...
    def read_last_month_leaderboard(self):
        ...
        with Session(self.engine) as session:
            ...
            query_result = leaderboard_controller.read_monthly(
                session=session, fdom=LLCMonth(previous=True).first_day_of_month()
            )
            for res in query_result:
                result.append({**res.User.as_dict(), **res.UserMonthlyObject.as_dict()})
        return result
    ...
    def read_daily_object(self, date):
        ...
        with Session(self.engine) as session:
            daily_object_controller = ctrlers.DailyObjectController()
            daily = daily_object_controller.read_one_or_latest(
    ...
    def read_gimme(self, lc_query):
        ...
        with Session(self.engine) as session:
            difficulty = None
            if "difficulty" in lc_query:
                difficulty = lc_query["difficulty"]
            premium = False
            if "premium" in lc_query:
                premium = lc_query["premium"]
            problems = ctrlers.ProblemController().read_many(
                session, difficulty, premium
            )
            topics_to_include = []
            topics_to_exclude = []
    ...
    def read_quiz(self, quiz_detail):
        ...
        with Session(self.engine) as session:
            difficulty = None
            category = None
            if "difficulty" in quiz_detail:
                difficulty = quiz_detail["difficulty"]
            if "category" in quiz_detail:
                category = quiz_detail["category"]
    ...
    async def update_score(self, memberDiscordId: str, delta: int, reason: str):
        ...
        with Session(self.engine) as session:
            user = ctrlers.UserController().read_one(session, discordId=memberDiscordId)
            daily_obj = ctrlers.DailyObjectController().read_one(
                session, date=get_today()
            )
            user_daily_object_controller = ctrlers.UserDailyObjectController()
            ...
            if user_daily_object is None:
                user_daily_object = user_daily_object_controller.create_one(
                    session, user.id, daily_obj.id, scoreEarned=delta
                )
            else:
            ...
            result = {
                "daily": user_daily_object.as_dict(),
                "monthly": monthly_obj.as_dict(),
            }
            await self.__commit(
                session,
    ...
    async def update_daily_quiz_score(self, memberDiscordId: str, delta: int):
        ...
        with Session(self.engine) as session:
            ...
            user = ctrlers.UserController().read_one(session, discordId=memberDiscordId)
            prev_daily_obj = ctrlers.DailyObjectController().read_one(
                session, date=prev_date
            )
            user_prev_daily_object_controller = ctrlers.UserDailyObjectController()
            user_prev_daily_object = user_prev_daily_object_controller.read_one(
                session, user.id, prev_daily_obj.id
            )
            if user_prev_daily_object is None:
                user_prev_daily_object = user_prev_daily_object_controller.create_one(
                    session, user.id, prev_daily_obj.id, scoreEarned=delta
                )
            else:
            ...
            if prev_date < LLCMonth().first_day_of_month():
                monthly_obj = ctrlers.UserMonthlyObjectController().updvate_one(
                    session,
                    user.id,
                    LLCMonth(previous=True).first_day_of_month(),
                    delta,
                )
            else:
                monthly_obj = ctrlers.UserMonthlyObjectController().update_one(
            ...
            result = {
                "daily": user_prev_daily_object.as_dict(),
                "monthly": monthly_obj.as_dict(),
            }
            await self.__commit(
                session,
                "Scoring",
                api_utils.score_update_jstr(
                    memberDiscordId,
                    delta,
                    f"Correct answer for daily quiz on {prev_date.strftime('%d/%m/%Y')}",
                ),
            )
        return result
    ...
    def read_profile(self, memberDiscordId: str):
        ...
        with Session(self.engine) as session:
            ...
            result = user.as_dict()
            daily_object = ctrlers.DailyObjectController().read_one_or_latest(
                session, date=get_today()
            )
            ...
            if user_monthly_object is None:
                result["monthly"] = {"scoreEarned": 0, "rank": "N/A"}
            else:
    ...
    async def create_user(self, user_obj: dict):
        ...
        with Session(self.engine, autoflush=False) as session:
            ...
            user_monthly_object_controller = ctrlers.UserMonthlyObjectController()
            user_monthly_object_controller.create_one(session, user.id)
    ...
    async def delete_old_account(self, user_id: int):
        with Session(self.engine) as session:
            ...
            try:
                await self.__commit(session, "Users<delete_count:1>", "[]")
            except Exception as e:
                print(e)
        return
    ...
    async def refresh_server_scores(self, firstDayOfMonth: datetime.date):
        with Session(self.engine, autoflush=False) as session:
            ...
            await self.__commit(
                session, f"UserMonthlyObject<count:{len(all_users)}>", "[]"
            )
        return
    ...
    def read_problems_all(self):
        ...
        with Session(self.engine) as session:
            ...
            result = list(
                map(
                    lambda x: x.as_dict(), problem_controller.read_many(session=session)
                )
            )
        return result
    ...
    async def create_problems(self, problems_list: List[dict]):
        ...
        with Session(self.engine, autoflush=False) as session:
            ...
            for problem in problems_list:
                topic_list = list(
                    map(lambda topic: topic["name"], problem["topicTags"])
                )
    ...
    async def update_submission_channel(self, new_channel_id: str):
        ...
        with Session(self.engine) as session:
            sys_conf_controller = ctrlers.SystemConfigurationController()
            result = sys_conf_controller.update(
                session=session, submissionChannelId=new_channel_id
            )
    ...
    async def update_score_channel(self, new_channel_id: str):
        ...
        with Session(self.engine) as session:
            sys_conf_controller = ctrlers.SystemConfigurationController()
            result = sys_conf_controller.update(
                session=session, scoreLogChannelId=new_channel_id
            )
            result = result.SystemConfiguration.as_dict()
            await self.__commit(session, "SystemConfiguration", json.dumps(result))
        return result
    ...
    async def update_contest_id(
        self, new_weekly_id: Optional[int] = None, new_biweekly_id: Optional[int] = None
    ):
        result = {}
        with Session(self.engine) as session:
            controller = ctrlers.ContestConfigurationController()
            result = controller.update(
                session=session,
                weeklyContestId=new_weekly_id,
                biweeklyContestId=new_biweekly_id,
            )
