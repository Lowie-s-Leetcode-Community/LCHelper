...
class UserDailyObjectController:
    ...
    def create_one(
        ...
        dailyObjectId: int,
        scoreEarned: int,
        solvedDaily: Optional[int] = 0,
        solvedEasy: Optional[int] = 0,
        solvedMedium: Optional[int] = 0,
        solvedHard: Optional[int] = 0,
    ...
    def update_one(
        ...
        userId: int,
        dailyObjectId: int,
        scoreEarnedDelta: int,
        solvedDailyDelta: Optional[int] = None,
        solvedEasyDelta: Optional[int] = None,
        solvedMediumDelta: Optional[int] = None,
        solvedHardDelta: Optional[int] = None,
        scoreGacha: Optional[int] = None,
    ):
        result = self.read_one(
            session=session, userId=userId, dailyObjectId=dailyObjectId
        )
        update_query = (
            update(db.UserDailyObject)
            .returning(db.UserDailyObject)
            .where(db.UserDailyObject.userId == userId)
            .where(db.UserDailyObject.dailyObjectId == dailyObjectId)
            .values(scoreEarned=result.scoreEarned + scoreEarnedDelta)
        )
        if solvedDailyDelta is not None:
            update_query = update_query.values(
                solvedDaily=result.solvedDaily + solvedDailyDelta
            )
        if solvedEasyDelta is not None:
            update_query = update_query.values(
                solvedEasy=result.solvedEasy + solvedEasyDelta
            )
        if solvedMediumDelta is not None:
            update_query = update_query.values(
                solvedMedium=result.solvedMedium + solvedMediumDelta
            )
        if solvedHardDelta is not None:
            update_query = update_query.values(
                solvedHard=result.solvedHard + solvedHardDelta
            )
        if scoreGacha is not None:
...
class UserMonthlyObjectController:
    ...
    def read_one(
        ...
        session: Session,
        userId: int,
        fdom: date = LLCMonth().first_day_of_month(),
    ):
        query = (
            select(db.UserMonthlyObject)
    ...
    def update_one(
        self, session: Session, userId: int, fdom: date, scoreEarnedDelta: int
    ):
        result = self.read_one(session=session, userId=userId, fdom=fdom)
        insp = inspect(result)
        if insp.persistent:
            update_query = (
                update(db.UserMonthlyObject)
...
class UserSolvedProblemController:
    def read_all(
        self, session: Session, userId: Optional[int], problemId: Optional[int]
    ):
...
class UserController:
    ...
    def read_one(
        ...
        elif leetcodeUsername is not None:
    ...
    def remove_one(self, session: Session, userId: int):
        ...
        query = delete(db.UserMonthlyObject).where(
            db.UserMonthlyObject.userId == userId
        )
        session.execute(query)
        query = delete(db.UserSolvedProblem).where(
            db.UserSolvedProblem.userId == userId
        )
        session.execute(query)
...
class TopicController:
    ...
    def create_one(self, session: Session, topicName: str):
        result = db.Topic(
            id=get_min_available_id(session, db.Topic), topicName=topicName
        )
        session.add(result)
        return result
...
class SystemConfigurationController:
    ...
    def update(
        self,
        session: Session,
        verifiedRoleId: Optional[str] = None,
        unverifiedRoleId: Optional[str] = None,
        timeBeforeKick: Optional[str] = None,
        dailySolveScore: Optional[int] = None,
        easySolveScore: Optional[int] = None,
        mediumSolveScore: Optional[int] = None,
        hardSolveScore: Optional[int] = None,
        practiceScoreCap: Optional[int] = None,
        streakBonus: Optional[int] = None,
        submissionChannelId: Optional[str] = None,
        scoreLogChannelId: Optional[str] = None,
        dailyThreadChannelId: Optional[str] = None,
        devErrorLogId: Optional[str] = None,
        databaseLogId: Optional[str] = None,
        backupChannelId: Optional[str] = None,
        eventLoggingId: Optional[str] = None,
    ):
        query = (
            update(db.SystemConfiguration)
            .returning(db.SystemConfiguration)
            .where(db.SystemConfiguration.id == 1)
        )
        if verifiedRoleId is not None:
...
class LeaderboardController:
    def read_monthly(
        self, session: Session, fdom: date = LLCMonth().first_day_of_month()
    ):
        query = (
            select(db.UserMonthlyObject, db.User)
            .join_from(db.UserMonthlyObject, db.User)
            .where(db.UserMonthlyObject.firstDayOfMonth == fdom)
    ...
    def read_daily(self, session: Session, dailyObjectId: int):
        query = (
            select(db.UserDailyObject, db.User)
            .join_from(db.UserDailyObject, db.User)
            .where(db.UserDailyObject.id == dailyObjectId)
...
class QuizController:
    def read_one(self, session: Session, quesId: Optional[int] = None):
        query = select(db.DiscordQuiz)
        if quesId is not None:
            query = query.where(db.DiscordQuiz.id == quesId)
        return session.scalar(query)

    def read_quiz_answer(self, session: Session, quesId: Optional[int] = None):
...
class ContestConfigurationController:
    ...
    def update(self, session, weeklyContestId=None, biweeklyContestId=None):
        ...
        if weeklyContestId is not None:
            query = query.values(weeklyContestId=weeklyContestId)
        if biweeklyContestId is not None:
