...
def get_min_available_id(session, Table):
    ...
    if table_name not in session.db_remaining_ids:
        query = (
            select((Table.id + 1).label("min_id"))
            .where(~(Table.id + 1).in_(select(Table.id)))
            .order_by("min_id")
        )
        session.db_remaining_ids[table_name] = list(
            map(lambda x: x.min_id, session.execute(query).all())
        )
    rank = count_obj_in_session(session, Table) + 1
    while rank > len(session.db_remaining_ids[table_name]):
        session.db_remaining_ids[table_name].append(
            session.db_remaining_ids[table_name][-1] + 1
        )
