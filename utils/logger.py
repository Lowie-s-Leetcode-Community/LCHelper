...
class Logger:
    ...
    async def on_db_update(self, success, context, message):
        try:
            ...
            if success:
                ...
                """
                embed = discord.Embed(description=message, color=Assets.easy)
                ...
                """
                embed = discord.Embed(description=message, color=Assets.hard)
                await log_channel.send(msg, embed=embed)
        except Exception:
    ...
    async def on_automation_event(self, context: str, message: str):
        try:
            ...
            log_channel = await guild.fetch_channel(
                self.client.config["eventLoggingId"]
            )
            await log_channel.send(
                f"**{context}** automation invoke: **{message}**. Timestamp: **{datetime.now()}**"
            )
        except Exception:
            print(traceback.format_exc()[:800])
            raise
    ...
    async def on_submission(self, user, problem, submission, is_daily):
        ...
        await log_channel.send(":new: AC Alert!", embed=embed)
