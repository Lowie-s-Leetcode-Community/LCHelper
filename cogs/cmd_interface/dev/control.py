...
class control(commands.Cog):
    ...
    async def _reload(self, ctx, *, s: str):
        ...
        if s == "all":
            ...
            if msg != "":
                ...
                await ctx.send(
                    f"{Assets.red_tick} **None of the provided cog(s) exist**"
                )
    ...
    async def sync(
        ...
        spec: Optional[Literal["1", "2", "3", "4"]] = None,
    ):
        if not guilds:
            if spec == "1":  # Syncing all commands in the current tree to local
                ...
                spec == "4"
            ):  # Clearing all commands on global and sync to local (to prevent duplicate commands)
...
async def setup(client):
    await client.add_cog(
        control(client), guilds=[discord.Object(id=734653818163298355)]
    )
