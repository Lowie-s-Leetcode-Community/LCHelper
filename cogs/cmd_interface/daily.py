...
class Daily(commands.Cog):
    ...
    @app_commands.command(
        name="daily", description="Returns Leetcode's Daily Challenge"
    )
    async def _daily(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(
            f"Daily Challenge - {display_date}", embed=embed
        )


async def setup(client):
    await client.add_cog(
        Daily(client), guilds=[discord.Object(id=client.config["serverId"])]
    )
