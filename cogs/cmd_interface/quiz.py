...
def createEmbed(_question: None, _answer: None, choice: int = -1):
    ...
    embed.add_field(
        name="Topic", value=f"||{question.category}||".ljust(10), inline=True
    )
    ...
    embed.add_field(name="Question ID".ljust(10), value=question.id, inline=True)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.is_correct:
            ...
            embed.add_field(
                ...
                + keyAns[self.button_type]
                + " But the correct answer is "
                + keyAns[self.view.correct_answer],
                value="Found an error in this question? Please, let us know in <#1085444549666680906>!",
                inline=False,
            )
        embed.add_field(
...
class Quiz(commands.Cog):
    ...
    async def _quiz(
        ...
        await interaction.followup.send(
            embed=createEmbed(quiz_result[0], quiz_result[1]), view=view
        )
...
async def setup(client):
    await client.add_cog(
        Quiz(client), guilds=[discord.Object(id=client.config["serverId"])]
    )
