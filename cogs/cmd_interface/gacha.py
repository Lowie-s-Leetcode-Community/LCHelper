...
GACHA_RESPONSE_LIST = [
    ...
    {"response": "Giảng viên thông báo cuối kỳ thi được dùng tài liệu.", "score": 1},
    {
    ...
    {"response": "Bạn đi thi kết thúc học phần và trúng tủ.", "score": 2},
    {
...
class Gacha(commands.Cog):
    ...
    @app_commands.command(
        name="gacha",
        description="Thử thách độ :turtle: của bạn sau khi hoàn thành nhiệm vụ!",
    )
    async def _gacha(self, interaction: discord.Interaction):
        avatar_url = (
            interaction.user.guild_avatar.url
            if interaction.user.guild_avatar
        ...
        if score_earned < 2:
            ...
            embed.set_author(
                name="Kết quả quay Gacha: Chưa hoàn thành nhiệm vụ!",
            ...
            embed.set_author(
                name="Kết quả quay Gacha! Hẹn mai nhé :)",
                icon_url=self.client.user.display_avatar.url,
            )
        else:
            ...
            embed = None
            try:
                await self.client.db_api.update_gacha_score(
                    str(interaction.user.id), roll["score"]
                )
            except Exception:
                embed = discord.Embed(
                ...
                embed.set_author(
                    name="Lỗi!", icon_url=self.client.user.display_avatar.url
                )
            else:
                embed = discord.Embed(
...
async def setup(client):
    await client.add_cog(
        Gacha(client), guilds=[discord.Object(id=client.config["serverId"])]
    )
