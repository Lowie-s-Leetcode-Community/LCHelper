...
class Task(commands.Cog):
    ...
    async def _task(self, interaction: discord.Interaction):
        ...
        score_msg += (
            f"- This month's score: **{user_progress['monthly']['scoreEarned']}**\n"
        )
        ...
        daily_msg = ""
        if (
            user_progress["user_daily"]["scoreEarned"] is None
            or user_progress["user_daily"]["scoreEarned"] < 0
        ):
        ...
        if (
            user_progress["user_daily"]["scoreGacha"] is None
            or user_progress["user_daily"]["scoreGacha"] < 0
        ):
        ...
        solved_easy = (
            user_progress["user_daily"]["solvedEasy"]
            if user_progress["user_daily"]["solvedEasy"] is not None
            else 0
        )
        solved_medium = (
            user_progress["user_daily"]["solvedMedium"]
            if user_progress["user_daily"]["solvedMedium"] is not None
            else 0
        )
        solved_hard = (
            user_progress["user_daily"]["solvedHard"]
            if user_progress["user_daily"]["solvedHard"] is not None
            else 0
        )

        practice_score = min(
            daily_conf["practiceCap"],
            solved_easy * daily_conf["easy"]
            + solved_medium * daily_conf["medium"]
            + solved_hard * daily_conf["hard"],
        )
        ...
        embed.add_field(name="Daily Progress", value=daily_msg, inline=False)
...
async def setup(client):
    await client.add_cog(
        Task(client), guilds=[discord.Object(id=client.config["serverId"])]
    )
