...
def create_embed(_question: dict,
                 _answer: dict,
                 score,
                 idx_quiz,
                 num_quiz,
                 choice: int = UNANSWERED):
    question = _question
    answers = _answer
    ...
    embed.add_field(
        name=f"Score",
        value=f"{SCORE_FOR_DIFFICULTY[question.difficulty]}",
        inline=True
    )

    embed.add_field(
        name=f"Question",
        value=f"{idx_quiz + 1}/{num_quiz}",
        inline=True
    )
...
class NavButton(discord.ui.Button['MultiQuizView']):
    def __init__(self, button_type: int,
                 style: discord.ButtonStyle, is_disabled: bool = False,
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.button_type == NEXT:
            ...
            await interaction.followup.send(
                embed=create_result_embed(self.view.score, self.view.list_quiz, self.view.answer_his))
        ...
        embed = create_embed(_question=self.view.list_quiz[self.view.idx_quiz][0],
                             ...
                             choice=choice)
        view = MultiQuizView(list_quiz=self.view.list_quiz,
                             idx_quiz=self.view.idx_quiz,
                             score=self.view.score,
...
class AnswerButton(discord.ui.Button['MultiQuizView']):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = create_embed(_question=self.view.list_quiz[self.view.idx_quiz][0],
                             _answer=self.view.list_quiz[self.view.idx_quiz][1],
                             score=self.view.score +
                                   SCORE_FOR_DIFFICULTY[self.view.list_quiz[self.view.idx_quiz][0].difficulty]
                             if self.is_correct else self.view.score,
                             idx_quiz=self.view.idx_quiz,
                             num_quiz=len(self.view.list_quiz),
        ...
        if self.is_correct:
            ...
            self.view.score = self.view.score + SCORE_FOR_DIFFICULTY[
                self.view.list_quiz[self.view.idx_quiz][0].difficulty]
        else:
            embed.add_field(
                name="Wrong! You chose " + keyAns[self.button_type] + " But the correct answer is ",
                value='Found an error in this question? Please, let us know in <#1085444549666680906>!',
                inline=False
            )
...
class MultiQuizView(discord.ui.View):
    ...
    def __init__(self, list_quiz: list, idx_quiz: int, score: int, end_quiz, answer_his=None, user=None):
        super().__init__()
        ...
        self.add_item(NavButton(button_type=1, style=discord.ButtonStyle.blurple, label="< Prev"))
        self.add_item(NavButton(button_type=3, style=discord.ButtonStyle.blurple, label="Next >"))

        for i in range(len(self.list_quiz[self.idx_quiz][1])):
            self.add_item(AnswerButton(button_type=i,
                                       is_correct=(i == self.list_quiz[self.idx_quiz][0].correctAnswerId
                                                   - self.list_quiz[self.idx_quiz][1][0].id),
                                       style=discord.ButtonStyle.gray,
                                       is_disabled=False,
                                       emoji=iconKey[i]))

        self.add_item(NavButton(button_type=4, style=discord.ButtonStyle.green, label="End Quiz"))

        if (self.answer_his[self.idx_quiz] != UNANSWERED
...
class Quiztest(commands.Cog):
    ...
    async def _quiz_test(self,
                         interaction: discord.Interaction,
                         nums_of_quiz: app_commands.Choice[int]):
        await interaction.response.defer(thinking=True)
        questions = []
        question_ids = {}
        difficulties = [("Easy", int(nums_of_quiz.value * 0.4)),
        ...
        embed = create_embed(_question=questions[0][0],
        ...
        for remaining in range(countdown_time, 0, -1):
            ...
            await countdown_message.edit(content=f"⏰ Time left: {remaining} seconds \n"
...
async def setup(client):
    await client.add_cog(Quiztest(client), guilds=[discord.Object(id=client.config['serverId'])])
