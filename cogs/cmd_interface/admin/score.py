...
class Score(commands.Cog):
    ...
    async def _score_add(
        ...
        if score <= 0:
            await interaction.followup.send(
                f"{Assets.red_tick} **`Score` should be positive. Use `/score deduct` instead.**"
            )
            return
    ...
    async def _score_deduct(
        ...
        if score <= 0:
            await interaction.followup.send(
                f"{Assets.red_tick} **`Score` should be positive. Use `/score add` instead.**"
            )
            return
...
async def setup(client):
    await client.add_cog(
        Score(client), guilds=[discord.Object(id=client.config["serverId"])]
    )
