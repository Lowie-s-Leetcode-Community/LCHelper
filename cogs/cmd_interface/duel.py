...
class DuelButton(discord.ui.Button['DuelView']):
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user != self.opponent:
            await interaction.response.send_message("Only the person challenged can interact with this button.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True)
        await self.view.handle_choice(self.label.lower())

class DuelView(discord.ui.View):
    def __init__(self, opponent: discord.Member):
        super().__init__(timeout=120) # 2 minutes each duel request
        self.response = None
...
class Duel(commands.Cog):
    def __init__(self, client):
        ...
        self.players = []
        self.duel_timeout = 600  # 10 minutes each duel
    ...
    async def duel(self, interaction: discord.Interaction, opponent: discord.Member):
        if not self.client.db_api.read_profile(memberDiscordId=str(interaction.user.id)):
            await interaction.response.send_message("Only verified member can duel. Type `/verify` to (re)verify yourself immediately!")
            return
        if not self.client.db_api.read_profile(memberDiscordId=str(opponent.id)):
            await interaction.response.send_message(f"The opponent {opponent.mention} is not verified and cannot participate in the duel.")
            return
        ...
        if self.is_duel_active:
            if self.current_problem == None:
                await interaction.response.send_message(
                    f"A duel request between {self.players[0].mention} and {self.players[1].mention} is already in progress. Try again later."
                )
                return
            else:
        ...
        await interaction.response.send_message(f"{opponent.mention}, you have been challenged to a duel by {interaction.user.mention}. Do you accept?", view=view)
        await view.wait()
        ...
        if view.response is None:
            ...
            # Start the timer
            self.duel_timeout_task = asyncio.create_task(self.duel_timeout_handler(interaction=interaction))
        else:
            await interaction.followup.send(f"{opponent.mention} has declined the duel request.")
            self.reset_duel()

    async def duel_timeout_handler(self, interaction: discord.Interaction):
        await asyncio.sleep(self.duel_timeout)
        if self.is_duel_active:
            await interaction.followup.send("The duel has ended due to timeout.")
            self.reset_duel()

    async def check_duel(self, interaction: discord.Interaction):
    ...
    async def check_solution(self, interaction: discord.Interaction):
    ...
    async def submit(self, interaction: discord.Interaction):
        if not await self.check_duel(interaction):
            return

        if await self.check_solution(interaction):
    ...
    async def propose_draw(self, interaction: discord.Interaction):
        ...
        view = DuelView(opponent)
        await interaction.response.send_message(f"{opponent.mention}, {proposer.mention} has proposed a draw. Do you accept?", view=view)
        await view.wait()

        if view.response is None:
            await interaction.followup.send("Draw request timed out.")
        elif view.response:
            await interaction.followup.send(f"The duel between {proposer.mention} and {opponent.mention} has been drawn. End the duel!")
            self.reset_duel()
        else:
