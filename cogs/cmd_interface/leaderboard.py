...
from lib.embed.interactable_leaderboard_embed import (
    InteractableLeaderboardEmbed,
    RankingView,
)
...
class Leaderboard(commands.Cog):
    ...
    async def _leaderboard_current(
        self, interaction: discord.Interaction, by_role: app_commands.Choice[str] = None
    ):
        ...
        user_list, title = await self.filter_by_role(
            interaction, user_list, by_role, title
        )
        ...
        embed = InteractableLeaderboardEmbed(
            title, user_list, 1, pages_count, embed_limit, interaction
        )
    ...
    @rank_group.command(
        name="previous", description="Take a look at LLC's previous Hall of Fame"
    )
    @app_commands.describe(by_role="User sorted by role")
    @app_commands.choices(
        by_role=[app_commands.Choice(name=role, value=role) for role in role_list]
    )
    async def _leaderboard_previous(
        self, interaction: discord.Interaction, by_role: app_commands.Choice[str] = None
    ):
        await interaction.response.defer(thinking=True)
    ...
    async def filter_by_role(self, interaction, user_list, by_role, title):
        if by_role is not None:
            ...
            user_list = [
                ...
                if role in guild.get_member(int(user["discordId"])).roles
            ]
...
async def setup(client):
    await client.add_cog(
        Leaderboard(client), guilds=[discord.Object(id=client.config["serverId"])]
    )
