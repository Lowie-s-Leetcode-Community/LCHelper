...
from utils.llc_datetime import LLCMonth, get_date_from_timestamp
...
class Crawl(commands.Cog):
    ...
    async def crawling(self):
        ...
        if current_utc_time.hour == 12 and (
            datetime.today() == 4 or datetime.today() == 6
        ):
            await self.logger.on_automation_event(
                "Crawl", "No crawl to generate weekly form"
            )
            return

        if current_utc_time.hour == 0:
            await self.logger.on_automation_event(
                "Crawl", "No crawl to avoid conflict with other tasks."
            )
            return
...
async def setup(client):
    await client.add_cog(
        Crawl(client), guilds=[discord.Object(id=client.config["serverId"])]
    )
