...
from utils.llc_datetime import get_next_LLC_week_and_month, next_weekday
from utils.logger import Logger

COG_START_TIMES = [datetime.time(hour=12, minute=00, tzinfo=datetime.timezone.utc)]
...
class WeeklyAutomation(commands.Cog):
    ...
    def backtrack_get_member(
        self,
        candidates_in_day: list,
        current_list: list,
        result: list,
        frequency: dict,
        day: int,
        ...
        if len(candidates_in_day[day]) == 0:
            current_list.append({})
            self.backtrack_get_member(
                candidates_in_day, current_list, result, frequency, day + 1, min_empty
            )
            current_list.pop()
            return
    ...
    async def create_weekly_thread(self):
        ...
        message = (
            f"Form đăng ký chữa Daily tuần {week} tháng {month} đã chính thức mở. "
    ...
    async def get_member_solve_problem(self):
            ...
            async for user in reaction.users():
                ...
                if user.id in candidates:
                    ...
                    candidate = await self.get_candidate(
                        user.id, user.name, member_roles, guild
                    )
        ...
        min_empty = [7]
        self.backtrack_get_member(
            candidates_for_day, current_list, result, frequency, 0, min_empty
        )
    ...
    @weekend.error
    async def on_weekend_error(self, exception):
        guild = await self.client.fetch_guild(self.client.config["serverId"])
        channel = await guild.fetch_channel(self.client.config["serverId"])
        await channel.send(
            f"Weekend initiate error:\n```py\n{traceback.format_exc()[:800]}```\nPlease re-start."
        )
    ...
    @app_commands.command(
        name="weekly_simulate", description="Simulate a weekly crawl cycle."
    )
    ...
    @app_commands.command(
        name="gen_weekly_member", description="Simulate a weekly crawl cycle."
    )
    @app_commands.checks.has_permissions(administrator=True)
    async def _gen_mem(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(
            f"{Assets.green_tick} **Weekly Task 2 finished**"
        )


async def setup(client):
    await client.add_cog(
        WeeklyAutomation(client), guilds=[discord.Object(id=client.config["serverId"])]
    )
